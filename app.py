...
def evaluate_round_and_advance():
    """v3.1: Ocena bieżącej rundy PRZED przejściem dalej.
      ...
      - następnie gra przechodzi do kolejnej rundy / scenariusza / kończy się.
    Zwraca podsumowanie: { "passed": [...], "eliminated": [...], "threshold": float }."""
    s_idx = state["scenario_idx"]
    r_num = state["round"]
    scen = state["scenarios"][s_idx]
    current_round = scen["rounds"].get(r_num) or scen["rounds"].get(str(r_num))

    summary = {"passed": [], "eliminated": [], "threshold": 0.0, "bonus": {}}
    ...
    # Przejście do kolejnej fazy (dotychczasowa logika)
    ...
    with tab1:
        ...
        if st.button("⏩ ZAKOŃCZ RUNDĘ, ROZDAJ BONUSY I AKTYWUJ NASTĘPNY ETAP", use_container_width=True):
            ...
            if summary["eliminated"]:
                st.error(f"☠️ Wyeliminowane (kurs < {summary['threshold']:.1f}): {', '.join(summary['eliminated'])}")
            time.sleep(1.2)
            st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
...
def center_panel_fragment(team_name):
    ...
    if team_data.get("is_eliminated"):
        ...
        st.error(f"☠️ ZESPÓŁ WYELIMINOWANY W RUNDZIE {elim_r}. Zbyt niski kurs akcji / wskaźniki krytyczne.")
